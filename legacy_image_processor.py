...
class Preprocessor:
    def __init__(self, background_img="images/background.png"):
        ...
        self.bg_img = cv2.imread(background_img, cv2.IMREAD_COLOR)[self.roi_area[1]:self.roi_area[3],
                      self.roi_area[0]:self.roi_area[2]]
    ...
    def remove_background(self, input_img):
        ...
        delta = np.where(np.reshape(np.logical_and(mask1, mask2), inp_h.shape + (1,)),
                         np.array([0, 0, 0], dtype=np.uint8), input_img)
    ...
    def cluster_lines(self, input_img):
        ...
        self.cluster_hough_lines = cv2.HoughLinesP(input_img, rho=7, theta=np.pi / 180, threshold=10, minLineLength=8,
                                                   maxLineGap=2)
    ...
    def cluster_find_neighbor(self, line_obj):
        ...
        for lt in self.cluster_linelist:
            if self.distance(line_obj[1], lt[1]) <= self.cluster_line_maxdist and lt[
                1] not in self.cluster_in_class and not (
                    line_obj[0] == lt[0]).all():
    ...
    def find_text_contour_hierachy(self, input_img):
        ...
        for contour in contours:
            ...
            self.cluster_contours.append((contour_points, (int(center_x / len(points)), int(center_y / len(points))),
                                          (lowest_x, lowest_y), contour))
    ...
    def cluster_contour_find_neighbor(self, cont):
        ...
        for lt in self.cluster_contours:
            if self.distance(cont[1], lt[1]) <= self.cluster_contour_maxdist and lt[
                1] not in self.cluster_contour_centroids_in_class and not (
                    cont[2] == lt[2]):
                return_list.append(lt)
        return return_list
